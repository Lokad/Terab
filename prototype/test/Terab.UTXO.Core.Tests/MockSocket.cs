    ...
    public class MockSocket : ISocketLike
    {
        ...
        private void CheckExpectedCall(Expected expected, Method actualCalled)
        {
            Assert.True(expected.Called == actualCalled,
                $"{actualCalled} called" + Environment.NewLine + $"   Instead, a call to {expected.Called} has been setup at {expected.Source}:{expected.Line}");
        }
        ...
        public void ExpectSend(SpanToInt f, [CallerFilePath] string fileName=null, [CallerLineNumber] int lineno = 0) => Expect(Method.Send, f, fileName, lineno);
        ...
        public void ExpectAvailable(Func<int> f, [CallerFilePath] string fileName=null, [CallerLineNumber] int lineno = 0) => Expect(Method.Available, f, fileName, lineno);

        public void ExpectConnected(Func<bool> f, [CallerFilePath] string fileName=null, [CallerLineNumber] int lineno = 0) => Expect(Method.Connected, f, fileName, lineno);

        public void ExpectClose([CallerFilePath] string fileName=null, [CallerLineNumber] int lineno = 0) => Expect(Method.Close, null, fileName, lineno);
